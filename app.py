...
def save_data(df):
    if not df.empty:
        # 1. تحويل النصوص الفارغة والمساحات أو الكلمات الوهمية إلى قيم NaN حقيقية للتمكن من فحصها
        df = df.replace(r'^\s*$', pd.NA, regex=True)
        df = df.replace("None", pd.NA)
        df = df.replace("<NA>", pd.NA)
        df = df.replace("nan", pd.NA)
        
        # 2. حذف أي سطر يحتوي على خط رئيسي أو فرعي فارغ تماماً (حذف نظيف للممسوح)
        df = df.dropna(subset=["الخط الرئيسي", "الخط الفرعي"], how="any")
        
        # 3. تصفية نهائية للتأكد من أن الأرقام ليست فارغة بعد تحويل الأنواع
...
with tab1:
    ...
    df_clean_dropdown = df_clean_dropdown[
        ...
        (df_clean_dropdown["الخط الرئيسي"].astype(str) != "nan") & 
        (df_clean_dropdown["الخط الرئيسي"].astype(str) != "None")
    ]
...
with tab2:
    ...
    if not df_data.empty:
        ...
        with col_btn1:
            if not validation_passed:
                ...
                if st.button("💾 حفظ التعديلات", type="primary", key="save_editor"):
                    save_data(edited_df)
                    st.success("تم تحديث وتنظيف كافة البيانات بنجاح تام واختفت تماماً من كافة القوائم والـ Dropdowns!")
                    st.rerun()
        with col_btn2:
        ...
        st.success(f"الإجمالي الكلي الفعلي للمحصلات: **{actual_collected} ج.م**")
    else:
